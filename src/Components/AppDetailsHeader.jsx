...
const AppDetailsHeader = ({ appData }) => {
  const { id } = useParams();
  ...
  const { title, image, companyName, downloads, reviews, ratingAvg, size } =
    thisApp || {};
  ...
  return (
    <div className="grid grid-cols-12 grid-rows-3 gap-10">
      <div className="col-span-3  md:row-span-5 ">
      ...
      <div className="col-span-9 col-start-4">
        ...
        <p class="text-lg text-[#627382] ">
          Developed by{" "}
          <span class="text-gradient font-medium">{companyName}</span>
        </p>
      </div>
      <div className="col-span-12 md:col-span-9 row-span-2 md:row-span-3 md:col-start-4 row-start-2">
        <hr class="border-gray-300 mb-5" />

        <div class="flex items-center justify-baseline gap-15 mb-6">
        ...
        <div
          className={` ${isInstalled ? "md:w-30" : "md:w-70"} w-full overflow-hidden`}
        >
          <button
